...
void DeviceController::chargeDevicesThatNeedIt() {
    ...
    for (size_t i = 0; i < devices.size(); ++i) {
        ...
        // Новый порог зарядки: 50% вместо 20%
    ...
    // Сортировка по убыванию приоритета (стабильная)
...
void DeviceController::findDeviceByName() {
    std::cout << "Введите имя устройства: ";
    std::string name;
    std::getline(std::cin >> std::ws, name); // поддержка пробелов в имени

    bool found = false;
    for (size_t i = 0; i < devices.size(); ++i) {
        if (devices[i]->getName() == name) {
            std::cout << "Статус устройства '" << name << "':\n";
            devices[i]->printStatus(); // корректный вызов функции
            found = true;
            break;
        }
    }
    if (!found) {
        std::cout << "Устройство '" << name << "' не найдено" << std::endl;
    }
}
...
void DeviceController::printEnergyReport() const {
    ...
    // 1 ккал/ч ≈ 1.163 Вт → ккал = Вт·ч / 1.163
